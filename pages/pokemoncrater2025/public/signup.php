<?php
header('location:http://signup.php');
exit();
include('pv_connect_to_db.php');
	if(!isset($_REQUEST['ajax'])){
		...
		<meta name="description" content="Pok&eacute;mon Shqipe Battle Arena v3 RPG. A free online Pok&eacute; game where you can catch, battle and trade all of your favourite Pok&eacute;mon.">
		<title>Pok&eacute;mon Shqipe v3 - Sign Up</title>
		</head>
		<body>
		<div id="alert"></div>
		<div id="container">
		...
		<a href="http://facebook.com/pokemonshqipe" class="s">Shqipe Facebook</a><br />
				...
				if($_POST['terms'] != "agree"){
					...
					if($pass != $pass2){
						...
						if($username == "" || $email == "" || $pass == "" || $pass2 == ""){
							...
							elseif(strstr($username,"!") || strstr($username,"�") || strstr($username,"`") || strstr($username,"�") || strstr($username,"$") || strstr($username,"%") || strstr($username,"^") || strstr($username,"&") || strstr($username,"*") || strstr($username,")") || strstr($username,")") || strstr($username,"+") || strstr($username,"=") || strstr($username,".") || strstr($username,">") || strstr($username,"?") || strstr($username,"/") || strstr($username,"|") || strstr($username,"<") || strstr($username,",") || strstr($username," ") || strstr($username,"'") || strstr($username,'"') || strstr($username,"#") || strstr($username,"~") || strstr($username,"@") || strstr($username,":") || strstr($username,";") || strstr($username,"[") || strstr($username,"]") || strstr($username,"{") || strstr($username,"}") || strstr($username,",") || stristr($username,"�") || stristr($username,"�") || stristr($username,"�") || stristr($username,"�") || stristr($username,"�") || strstr($username,"�") || strstr($username,"=")){
								$error = 8;
							}
		...
		if($trainer < '1' || $trainer > '28'){
			...
			if($asd != 1){
				...
				if(mysql_num_rows($sell) == 0){
					...
					mysql_query("INSERT INTO flashchat_users (id, login, password, roles) VALUES ('$vb', '{$selly['user']}', '$pass', '1')");
					$gendrand = rand(1,2);
					if($gendrand == 1){
						$gender = Male;
					}
					...
					mysql_query("INSERT INTO pokemon (name, pid, a1, a2, a3, a4, lvl, t1, t2, exp, rowner, owner, ball, gender, ot) VALUES ('{$_POST['spoke']}', '{$row['id']}', '{$row['a1']}', '{$row['a2']}', '{$row['a3']}', '{$row['a4']}', '18', '{$row['type1']}', '{$row['type2']}', '9000', '{$selly['user']}', '{$vb}', 'Poke Ball', '$gender', '{$selly['user']}')");
					$h3 = mysql_insert_id();
						// Include the stat generating pages
					include('stats/ivs.php');
					include('stats/natures.php');
					include('stats/signupabilities.php');
					mysql_query("INSERT INTO pokemon_stats (id, hp_iv, attack_iv, defense_iv, spatk_iv, spdef_iv, speed_iv, nature, ability) VALUES ('$h3', '$hp_iv', '$attack_iv', '$defense_iv', '$spatk_iv', '$spdef_iv', '$speed_iv', '$nature', '$ability')");
					...
					echo '<meta http-equiv="Refresh" content="4; url=http://pokemon-shqipe.co.uk/login.php?reg=1">';
				}
			}
		}
	}
	if(isset($page) && $page == 2){
		...
		echo '<form method="post" action="sign.php"><div style="text-align: center; margin: 0 auto;"><h2 style="margin-top: 25px;">Step 2: Select sprite and starting pokemon</h2>';
	...
	if(isset($error) || !isset($_POST['submit']) && !isset($_POST['submitstep2'])){
		...
		<td style="text-align: left;"><input name="email" tabindex="4" alt="E-mail Address" type="text" id="email" size="40" maxlength="40" ';
